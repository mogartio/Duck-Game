...
protected:
    ...
    Player player;
    PlayerTest():
            initial_coordinate(INITIAL_X, INITIAL_Y), player(this->initial_coordinate, stage) {}
};

TEST_F(PlayerTest, PlayerExists) {
    EXPECT_CALL(stage, is_valid_position(::testing::_, ::testing::_)).WillRepeatedly(Return(true));
...
TEST_F(PlayerTest, PlayerMovesHorizontallyGrounded) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        ASSERT_EQ(player.get_position().y, INITIAL_Y)
                << "Se mueve verticalmente cuando deberia hacerlo solo a la derecha";
    }
    for (int i = 0; i < 5; i++) {
        player.move(left);
...
TEST_F(PlayerTest, PlayerJump) {
    EXPECT_CALL(stage, is_valid_position(::testing::_, ::testing::_)).WillRepeatedly(Return(true));
    EXPECT_CALL(stage, should_fall(::testing::_)).WillRepeatedly(Return(true));
    std::string jump = "w";
    std::string nothing = "s";
    player.move(jump);
    ASSERT_EQ(player.get_position().x, INITIAL_X)
            << "Se mueve horizontalmente cuando deberia hacerlo solo para arriba";
...
TEST_F(PlayerTest, PlayerFall) {
    ...
    player.move(left);
    ASSERT_EQ(player.get_position().x, INITIAL_X - 1)
            << "Se mueve horizontalmente cuando deberia hacerlo solo para arriba";
