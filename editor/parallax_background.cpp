...
void ParallaxBackground::resizeImages() {
    ...
    pixmap = pixmap.scaled(parentSize, Qt::KeepAspectRatioByExpanding);
    label->setPixmap(pixmap);
    label->setGeometry(0, 0, parentSize.width(), parentSize.height());
    label_dup->setPixmap(pixmap);
    label_dup->setGeometry(parentSize.width(), 0, parentSize.width(), parentSize.height());
}