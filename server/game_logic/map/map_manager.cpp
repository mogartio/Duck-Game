...
void MapManager::load_maps() {
    ...
    // for each .yaml found in "maps" folder, extract the info and create a Map instance to add to the maps vector
    for (const auto& entry : std::filesystem::directory_iterator(maps_folder_path)) {
        ...
        // create map instance
        Map new_map = Map(new_map_name, rows, columns, matrix, players_spawn_sites, items_spawn_sites);
